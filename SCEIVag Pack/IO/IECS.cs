    ...
    public class IECS
    {
        ...
        public class Sset : Section
        {
            ...
            public class SampleSet
            {
                ...
                {
                    return (SampleSet)MemberwiseClone();
                }
                public bool CreatedNow = false;
        ...
        public class Prog : Section
        {
            ...
            public class Entry
            {
                public Entry Clone()
                {
                    // Clonando os campos simples (tipos primitivos)
                    Entry clone = (Entry)this.MemberwiseClone();

                    // Clonando os arrays ou objetos internos
                    clone.Extents = this.Extents.Select(e => e.Clone()).ToArray();
                ...
                {
                    // Método para clonar a classe Extension
                ...
                if (entryOrdened.Pack_Offset.ToString("X2") != "FFFFFFFF")
                    for (int i = 0; i < sCEI_Entries.Count; i++)
                    {
                            ...
                            .ELFPointer_Offset)
                        {
                            ...
                            sCEI_Entries[i].VAGStream_AftrProg_Offset = entryOrdened.scei_File._Header.SectionsSize;
                            ...
                            if (entryOrdened.Repeated_Indices != null && entryOrdened.Repeated_Indices.Count() > 0)
                            {
                                for (int r = 0; r < entryOrdened.Repeated_Indices.Count(); r++)
                                {
                                    ...
                                    sCEI_Entries[entryOrdened.Repeated_Indices[r]].VAGStream_AftrProg_Offset = entryOrdened.scei_File._Header.SectionsSize;
                                }
                            }
